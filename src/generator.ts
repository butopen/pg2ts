...
export function generateEnum(enumObject: EnumTypes): string[] {
  ...
  for (let enumNameRaw in enumObject) {
    ...
    enumStrings.push(
      `export type ${enumName} = ${enumObject[enumNameRaw]
        .map((v: string) => `'${v}'`)
        .join(" | ")}`
    );
  }
...
export const typescriptTablesConst = async (
  db: Database,
  tables: string[],
  tableSchema: string,
  mainPrefix = ""
  ...
  for (let t of tables) {
    ...
    for (const col in tableDefinition) columns.push(col);
    let f = `${t}: {tableName: "${t}", ${columns
      .map((c) => `${c}: "${t}.${c}"`)
      .join(",")}}`;
    fields.push(f);
  }
  return `export const ${camelCase(mainPrefix, {
    pascalCase: true,
  })}TablesData = {
...
export const typescriptTablesCRUD = async (
  db: Database,
  tables: string[],
  tableSchema: string,
  mainPrefix = ""
...
export const typescriptLookupForTables = (
  tables: string[],
  mainPrefix = ""
): string => {
  const types = tables.map(
    (t) => `${t}: ${camelCase(t, { pascalCase: true })}`
  );
...
export const typescriptOfSchema = async (
  db: Database,
  options = { schema: "", mainPrefix: "" }
): Promise<string> => {
  const schema = options.schema || (await db.getDefaultSchema());
  const tables = (await db.getSchemaTables(schema)).filter(
    (t) => !["pg_stat_statements", "pg_stat_statements_info"].includes(t)
  );
  ...
  const interfaces = await Promise.all(
    tables.map((table) =>
      typescriptOfTable(db, schema, table, jsonTypesToImport)
    )
  );
  ...
  const consts = await typescriptTablesConst(
    db,
    tables,
    schema,
    options.mainPrefix
...
export const mapPostgresTableDefinitionToType = (
  tableDefinition: TableDefinition,
...
export class PostgresDatabase implements Database {
  ...
  public async getTableDefinition(tableSchema: string, tableName: string) {
    ...
    return result.rows.reduce(
      (result, { column_name, udt_name, is_nullable, has_default }) => {
        result[column_name] = {
  ...
  public async getColumnComments(schemaName: string, tableName: string) {
    ...
    return commentsResult.rows.reduce(
